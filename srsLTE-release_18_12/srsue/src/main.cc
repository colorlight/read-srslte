...
int main(int argc, char* argv[])
{
  ...
  // generate a instance, create a ue object 
  // the ue related function is in ue.cc for example init
  ...
  // this will start the tremendous procedures of the init work of all layers
  // int radio mac phy rrc nas etc
  if (!ue->init(&args)) {
    exit(1);
  }

  //this part is about logging, metricshub, and I won't go to detail about it.
  ...
  // pthread_create is to create a thread ,and the input_loop thread is about logging, so we won't go through about it.
  pthread_t input;
  pthread_create(&input, NULL, &input_loop, &args);

  printf("Attaching UE...\n");
  // it seems that the ue start the attach procedure, and keep attaching as long as running
  // according the comment of attach_request, if the ue could attach correctly, will return true
  // so if flase, it will stay in the while loop, and it will start attach again.
  // if not running , or succeed in attach procedure, it will jump out of the while, and it means that
